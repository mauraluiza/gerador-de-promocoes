# Project Documentation: Promo Generator

## File Information
**File Path:** `c:\Users\maura\OneDrive\Documentos\webapp\promo_generator.html`
...
### Dependencies
*   **Google Fonts**: Uses 'Outfit' font family.
*   **Flatpickr**: Used for date selection (CSS and JS CDN links included, plus Portuguese localization).

### Structure
*   **Single Page Application**: All logic is contained within one HTML file.
*   **CSS**: Custom styling using CSS variables for colors and glassmorphism effects.
*   **Layout**:
    *   `app-container`: Main wrapper with a grid layout (2 columns on desktop, 1 on mobile).
    *   `input-section`: Contains the form for user input.
    *   `preview-section`: Displays the generated text and copy button.

### Input Fields
*   **Headline**: Text input for the promotion title.
*   **City**: Text input for destination.
*   **Accommodation**: Dropdown (Duplo, Triplo, Quádruplo, Individual).
*   **Dates**: Two inputs (`dateStart`, `dateEnd`) using Flatpickr.
*   **Days**: Number input, calculated automatically from dates.
*   **Food**: Dropdown (Café da manhã, All inclusive, Half board, Full board).
*   **Hotel**: Text input.
*   **Package Value**: Text input with real-time currency formatting.
*   **Price Per Person**: Text input, calculated automatically (50% of package value).
*   **Down Payment %**: Number input for percentage.
*   **Down Payment Value**: Text input, calculated (rounded up to nearest 50).
*   **Installments**: Dropdown (1x to 15x).
*   **Installment Value**: Text input, calculated (`(Total - DownPayment) / Installments`).

### JavaScript Logic
*   **`parseCurrency(value)`**: Converts Brazilian currency string to float.
*   **`formatCurrency(value)`**: Converts float to Brazilian currency string.
*   **`formatCurrencyInput(input)`**: Handles real-time input masking for currency fields (digits only -> currency format).
*   **`calculatePrices(e)`**:
    *   Triggered on input changes.
    *   Calculates `pricePerPerson`, `downPayment`, and `installmentValue`.
    *   Updates respective fields.
*   **`calculateDays()`**:
    *   Triggered on date changes.
    *   Calculates inclusive difference between start and end dates.
*   **`generateText()`**:
    *   Constructs the promotional message string using values from all fields.
    *   Handles conditional logic for headline and food descriptions.
    *   Updates the `previewArea`.
*   **`copyText()`**:
    *   Copies content of `previewArea` to clipboard.
    *   Provides visual feedback on the button.
*   **Event Listeners**:
    *   Attached to all inputs to trigger `generateText` and `calculatePrices` where appropriate.
    *   Ensures real-time updates of the preview.
